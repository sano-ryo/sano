...
<input type="text" placeholder="投稿内容" name="comment">
...
$dsn = 'データベース名';//データベース作成
$user = 'ユーザー名';
$password = 'パスワード';
$pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));

$sql = "CREATE TABLE IF NOT EXISTS tbsano"//テーブル作成
	." ("
...
if(!empty($_POST['submit'])){//送信ボタンがおされた場合
	if(!empty($_POST['comment'])){//入力情報の取得およびtxtファイルへの書き込み
		...
		$sql = $pdo -> prepare("INSERT INTO tbsano (name, comment, date) VALUES (:name, :comment, :date)");//データベースに書き込み
	...
	if(!empty($_POST['num'])){//削除欄に数字が入力された場合番号不一致で異なるtxtに書き込み
		...
		if(ctype_digit($delete)){
			if(!empty($_POST['pass1'])){
				$passin1=$_POST['pass1'];
				if($passin1==$pass1){//入力されたパスワードと比較
					
					echo $delete."を削除しました。"."<br>";
					
					$id = $delete;
					$sql = 'delete from tbsano where id=:id';
		...
		if(ctype_digit($edit)){//編集番号が記入されると番号一致の箇所を編集
			if(!empty($_POST['pass2'])){
				...
				if($passin2==$pass2){
					...
					$sql = 'update tbsano set name=:name,comment=:comment,date=:date where id=:id';
						$stmt = $pdo->prepare($sql);
...
$sql = 'SELECT * FROM tbsano';//プラウザ上に表記
	$stmt = $pdo->query($sql);
